...
// ─── Helper : extrait un message lisible depuis n'importe quelle erreur ────────
// ✅ FIX : les erreurs Supabase ne sont PAS des instances de Error,
//    donc `err instanceof Error` retourne false et le message se perdait.
function getErrorMessage(err: unknown): string {
  if (!err) return 'Erreur inconnue'
  if (typeof err === 'string') return err
  if (typeof err === 'object') {
    // Erreur Supabase : { message: string, code: string, ... }
...
export default function EditAdPage() {
  ...
  const handleAddImages = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (total > 8) {
        toast.error('Maximum 8 photos par annonce')
        return
      }
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        // ✅ FIX : pas de filtre user_id pour l'admin
        let query = supabase
          .from('ads')
          .update({
        ...
        // ✅ FIX : les erreurs Supabase ne sont pas des Error JS
        //    → on utilise getErrorMessage() pour extraire le message proprement
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              ...
              <p className="text-xs text-gray-400 mt-3">
                {allImages.length}/8 photos • JPG, PNG, WebP
              </p>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  value={form.quartier}
                  onChange={handleChange}
                  placeholder="Ex: Cocody Riviera, Zone 4..."
                  className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-transparent transition"
                />
              </div>

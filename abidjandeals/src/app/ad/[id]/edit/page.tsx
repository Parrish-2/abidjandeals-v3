...
import { AlertTriangle, ArrowLeft, ImagePlus, Loader2, Save, X } from 'lucide-react'
...
export default function EditAdPage() {
  ...
  const [quartiersForReg, setQuartiersForReg] = useState<string[]>([])

  const [form, setForm] = useState<AdForm>({
    title: '', description: '', price: '',
    city: '', quartier: '', etat: '', marque: '', tel: '', whatsapp: '',
  })

  // Charge les régions (communes) depuis la table locations
  useEffect(() => {
  ...
  // Charge les quartiers quand la ville/commune change
  useEffect(() => {
    if (!form.city) { setQuartiersForReg([]); return }
    supabase.from('locations').select('name')
      .eq('region', form.city).eq('is_active', true).order('name')
      .then(({ data }) => {
        if (data) setQuartiersForReg(data.map((d: any) => d.name))
      })
  }, [form.city])
  ...
  const handleChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
      ...
      setForm(prev => {
        ...
        // Reset quartier quand la commune change
  ...
  const handleAddImages = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (images.length + newFiles.length + files.length > 8) {
        toast.error('Maximum 8 photos par annonce'); return
      }
  ...
  // ✅ FIX : utilise le même bucket que le formulaire de publication (ad-photos)
  const uploadNewImages = useCallback(async (): Promise<string[]> => {
    ...
    for (const file of newFiles) {
      ...
      const { error } = await supabase.storage
        .from('ad-photos')  // ✅ même bucket que publier/page.tsx
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      if (!form.city) { toast.error('La commune est obligatoire'); return }
      ...
      try {
        ...
        let query = supabase.from('ads').update({
        ...
        toast.error(`Erreur : ${getErrorMessage(err)}`, { id: toastId })
      } finally {
        setSubmitting(false)
      }
  ...
  if (loading) return (
    <div className="min-h-screen flex flex-col bg-gray-50"><Navbar />
      <div className="flex-1 flex items-center justify-center"><Loader2 size={32} className="animate-spin text-orange-500" /></div>
      <Footer /></div>
  )
  ...
  if (notFound) return (
    <div className="min-h-screen flex flex-col bg-gray-50"><Navbar />
      <div className="flex-1 flex flex-col items-center justify-center gap-4 text-center px-4">
        ...
        <Link href="/" className="text-orange-500 font-semibold">Retour à l'accueil</Link>
      </div><Footer />
    </div>
  )
  ...
  if (forbidden) return (
    <div className="min-h-screen flex flex-col bg-gray-50"><Navbar />
      <div className="flex-1 flex flex-col items-center justify-center gap-4 text-center px-4">
        ...
        <Link href="/dashboard" className="text-orange-500 font-semibold">Mon tableau de bord</Link>
      </div><Footer />
    </div>
  )
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              ...
              <div className="grid grid-cols-3 gap-2">
                {images.map((src, i) => (
                  <ImageThumb key={`e-${i}`} src={src} onRemove={() => removeExistingImage(i)}
                    isPrimary={i === 0 && newPreviews.length === 0} />
                ))}
                {newPreviews.map((src, i) => (
                  <ImageThumb key={`n-${i}`} src={src} onRemove={() => removeNewImage(i)}
                ...
                {canAddMore && (
                  <label className="aspect-square rounded-xl border-2 border-dashed border-gray-200 bg-gray-50 hover:bg-orange-50 hover:border-orange-300 transition-all flex flex-col items-center justify-center gap-1 cursor-pointer">
                    ...
                    <input type="file" multiple accept="image/jpeg,image/png,image/webp"
                      onChange={handleAddImages} className="hidden" />
                  </label>
                )}
              </div>
              {uploading && (
              ...
              <p className="text-xs text-gray-400 mt-3">{allImages.length}/8 photos</p>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
              ...
              <div>
                ...
                <textarea name="description" value={form.description} onChange={handleChange}
                  rows={4} maxLength={2000} placeholder="Décrivez votre article..."
              ...
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1.5">Prix (FCFA) <span className="text-red-500">*</span></label>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
              ...
              {/* Commune — chargée depuis Supabase locations */}
              <div>
              ...
              <div>
                ...
                {quartiersForReg.length > 0 ? (
                  <select name="quartier" value={form.quartier} onChange={handleChange}
                    className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 transition bg-white">
                    <option value="">Sélectionner un quartier</option>
                    {quartiersForReg.map(q => <option key={q} value={q}>{q}</option>)}
                  </select>
                ) : (
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input name="tel" value={form.tel} onChange={handleChange} type="tel"
            ...
            <div className="flex gap-3 pb-6">
              ...
              <button type="submit" disabled={submitting || uploading}
                className="flex-1 py-3.5 rounded-2xl bg-orange-500 hover:bg-orange-600 disabled:opacity-60 text-white font-bold text-sm flex items-center justify-center gap-2 transition">
                {submitting ? <><Loader2 size={16} className="animate-spin" /> Sauvegarde...</> : <><Save size={16} /> Sauvegarder</>}
              </button>
            </div>
          </form>
        </div>

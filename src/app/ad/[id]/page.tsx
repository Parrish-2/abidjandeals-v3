...
const isVideoUrl = (url: string) => VIDEO_EXTS.some(ext => url.toLowerCase().endsWith(ext))

function filterMedia(raw: string[]): { images: string[]; extractedVideo: string | null } {
  const images = raw.filter(u => !isVideoUrl(u))
  const extractedVideo = raw.find(u => isVideoUrl(u)) ?? null
  return { images, extractedVideo }
}
...
export default function AdDetailPage() {
  ...
  // ── Filtrage médias ────────────────────────────────────────────────────
  const { images, extractedVideo } = filterMedia((ad.images ?? []) as string[])
  const finalVideoUrl = (ad.video_url as string | null) ?? extractedVideo
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              <div className="flex gap-2 justify-end">
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-xl border text-sm font-medium transition ${isFav

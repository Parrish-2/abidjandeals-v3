...
const SLUG_TO_DB_CATEGORY: Record<string, string> = {
  ...
  'cat_immo': 'cat_immo',
  'cat_mode': 'cat_mode',
  'cat_maison': 'cat_maison',
  'cat_serv': 'cat_serv',
  'cat_loisir': 'cat_loisir',
  'cat_autres': 'cat_autres',
  'cat_agri': 'cat_agri',
}
...
const LABELS: Record<string, string> = {
  ...
  "hightech": "High-Tech & Informatique",
  "hightech-informatique": "High-Tech & Informatique",
  "vehicules-equipements": "Véhicules & Équipements",
  "location": "Location & Mobilité",
  ...
  "lingerie": "Lingerie & Adulte",
  "cat_lingerie": "Lingerie & Adulte",
  "cat_auto": "Automobile & Industrie",
  "cat_tech": "High-Tech & Informatique",
  "cat_immo": "Immobilier",
  "cat_mode": "Mode & Beauté",
  "cat_maison": "Maison & Décoration",
  "cat_serv": "Services & Emploi",
  "cat_loisir": "Sport & Loisirs",
  "cat_autres": "Autres & Divers",
  "voitures-occasion": "Voitures d'occasion",
  "voitures-neuves": "Voitures Neuves",
  "motos-scooters": "Motos & Scooters",
  "pieces-pneus": "Pièces détachées & Pneus",
  ...
  "engins-chantier": "Équipements de Chantier",
  "smartphones": "Smartphones",
  "tablettes": "Téléphones & Tablettes",
  "ordinateurs": "Ordinateurs & Laptops",
  "tv-son": "TV & Home Cinéma",
  "photo-video": "Photo & Vidéo",
  "consoles-jeux": "Consoles & Jeux Vidéo",
  "objets-connectes": "Objets Connectés",
  "composants": "Composants (RAM, SSD, Cartes Mères)",
  "imprimantes": "Imprimantes & Scanners",
  "vente-appartement": "Location Appartements",
  "vente-maison-villa": "Vente Maisons & Villas",
  "location-meublee": "Location Meublée",
  "location-vide": "Location Vide",
  "colocation": "Colocation",
  "terrains": "Terrains avec ACD",
  "bureaux-boutiques": "Bureaux & Commerces",
  "vetements": "Vêtements & Chaussures",
  "chaussures": "Chaussures",
  "sacs-accessoires": "Sacs & Accessoires",
  "montres": "Montres & Bijoux",
  "cosmetiques": "Cosmétiques & Parfums",
  "meubles": "Meubles",
  "decoration": "Décoration",
  "jardin-bricolage": "Jardin & Bricolage",
  "offres-emploi": "Offres d'Emploi",
  "freelance-it": "Freelance IT & Design",
  "batiment": "BTP & Artisanat",
  "cours-formation": "Cours & Formations",
  "transport": "Transport & Livraison",
  "menage": "Ménage & Nettoyage",
  "securite": "Sécurité & Gardiennage",
  "evenementiel": "Événementiel",
  "equipements-sport": "Équipements de Sport",
  "instruments-musique": "Instruments de Musique",
  "jouets": "Jouets & Jeux",
  "voyages": "Voyages & Tourisme",
  "velos": "Vélos & Trottinettes",
  "animaux": "Animaux & Accessoires",
  "collection": "Objets de Collection",
...
function getLabel(slug: string | null): string {
  ...
  return LABELS[slug]
    ?? LABELS[resolveDbCategoryId(slug) ?? '']
      ...
      <Link href={`/ad/${ad.id}`}>
        <article className="group flex gap-4 bg-white border border-gray-100 rounded-2xl overflow-hidden hover:border-orange-200 hover:shadow-md transition-all duration-200 cursor-pointer">
          <div className="relative w-36 h-28 flex-shrink-0 bg-gray-50">
            {img ? (
              <img src={img} alt={ad.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" loading="lazy" />
            ) : (
              <div className="w-full h-full flex items-center justify-center text-3xl bg-gradient-to-br from-orange-50 to-amber-50">
                {getCatEmoji(ad.category_id)}
              </div>
            )}
            {isBoosted && (
              <span className="absolute top-2 left-2 bg-amber-400 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full uppercase flex items-center gap-1">
          ...
          <div className="flex-1 py-3 pr-4 min-w-0">
            ...
            <div className="flex items-center gap-3 mt-2">
              <span className="flex items-center gap-1 text-[11px] text-gray-400">
                <MapPin size={10} /> {ad.city}{ad.quartier ? `, ${ad.quartier}` : ""}
              </span>
    ...
    <Link href={`/ad/${ad.id}`}>
      <article className="group relative rounded-2xl bg-white border border-gray-100 overflow-hidden hover:border-orange-300 hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 cursor-pointer">
        ...
        <div className={`relative overflow-hidden bg-gray-50 ${isBoosted ? "pt-6" : ""}`} style={{ height: 188 }}>
          {img ? (
            <img src={img} alt={ad.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" onError={(e) => { (e.target as HTMLImageElement).style.display = "none" }} />
          ) : (
        ...
        <div className="p-3.5">
          <p className="text-[10px] text-orange-500 font-bold uppercase tracking-wider mb-1 truncate">
            {getLabel(ad.category_id)}{ad.marque ? ` - ${ad.marque}` : ""}
          </p>
          ...
          <div className="flex items-center justify-between mt-2.5 pt-2.5 border-t border-gray-50">
            <span className="flex items-center gap-1 text-[11px] text-gray-400 truncate">
              <MapPin size={10} className="flex-shrink-0" />{ad.city}{ad.quartier ? `, ${ad.quartier}` : ""}
            </span>
            <span className="flex items-center gap-1 text-[10px] text-gray-300 flex-shrink-0 ml-2">
    ...
    <div className="col-span-full">
      <div className="flex flex-col items-center justify-center py-20 text-center max-w-md mx-auto">
        <div className="relative mb-8">
          <div className="w-32 h-32 rounded-3xl bg-gradient-to-br from-orange-50 to-amber-100 flex items-center justify-center">
        ...
        <div className="flex flex-wrap gap-2 justify-center mb-6">
          {suggestions.map((s) => (
            <Link key={s} href={`/search?q=${encodeURIComponent(s)}`} className="px-3 py-1.5 bg-white border border-gray-200 text-gray-600 text-xs font-medium rounded-full hover:border-orange-400 hover:text-orange-500 transition-colors">{s}</Link>
          ))}
        </div>
...
function SearchContent() {
  ...
  // ─── Age Gate ──────────────────────────────────────────────────────────────
  // `ageCleared` : true si la catégorie n'est pas adulte, ou si l'âge est confirmé
  const isAdultCategory = ADULT_CATEGORIES.some(
    (a) => a === categorySlug || a === dbCategoryId
  )
  const [ageCleared, setAgeCleared] = useState<boolean>(
    !isAdultCategory || isAgeVerified()
  )
  const [showAgeGate, setShowAgeGate] = useState<boolean>(
    isAdultCategory && !isAgeVerified()
  )

  // Sync quand le slug change (navigation côté client sans rechargement)
  const prevCatRef = useRef(categorySlug)
  useEffect(() => {
    if (prevCatRef.current === categorySlug) return
    prevCatRef.current = categorySlug

    const adult = ADULT_CATEGORIES.some(
      (a) => a === categorySlug || a === resolveDbCategoryId(categorySlug)
    )
    if (adult && !isAgeVerified()) {
      setAgeCleared(false)
      setShowAgeGate(true)
    } else {
      setAgeCleared(true)
  ...
  const fetchAds = useCallback(async () => {
    // Ne pas charger les annonces tant que l'âge n'est pas confirmé
    if (!ageCleared) return

    setLoading(true)
    setError(null)
    try {
      let resolvedSubCatUuid: string | null = null
      ...
      if (subcategorySlug && dbCategoryId) {
        ...
        if (bySlug?.id) {
          ...
          const SLUG_TO_NAME: Record<string, string> = {
            ...
            'camions-utilitaires': 'Camions & Utilitaires',
            'groupes-electrogenes': 'Groupes Électrogènes',
            'outillage-industriel': 'Outillage Professionnel',
            'location-auto': 'Location Auto',
            'smartphones': 'Téléphones & Tablettes',
            'tablettes': 'Téléphones & Tablettes',
            'ordinateurs': 'Ordinateurs & Laptops',
            'tv-son': 'TV & Home Cinéma',
            'photo-video': 'Photo & Vidéo',
            ...
            'voyages': 'Voyages & Tourisme',
            'velos': 'Vélos & Trottinettes',
            'pharma': 'Parapharmacie & Santé',
            'epicerie': 'Épicerie & Boissons',
            'animaux': 'Animaux & Accessoires',
            'collection': 'Objets de Collection',
            'inclassables': 'Inclassables',
          }

          const nameToSearch = SLUG_TO_NAME[subcategorySlug]
          if (nameToSearch) {
            const { data: byName } = await supabase
              ...
              .limit(1)
              .maybeSingle()
            resolvedSubCatUuid = byName?.id ?? null
          }

          if (!resolvedSubCatUuid) {
            const keyword = subcategorySlug.replace(/-/g, " ").split(" ")[0]
            const { data: fallback } = await supabase
              ...
              .limit(1)
              .maybeSingle()
            resolvedSubCatUuid = fallback?.id ?? null
          }
        }
      }
      ...
      let query = supabase
        .from("ads")
        .select(
          `id, title, price, category_id, sub_category_id, etat, marque,
           city, quartier, images, boost_level, views, status, created_at`,
          { count: "exact" }
        )
        .eq("status", "active")
  ...
  // Relancer le fetch dès que l'âge est confirmé
  useEffect(() => { fetchAds() }, [fetchAds])

  const pageLabel = subcategorySlug
    ? getLabel(subcategorySlug)
    : categorySlug
      ? getLabel(categorySlug)
      : q
        ? `Résultats pour "${q}"`
        : "Toutes les annonces"

  const pageEmoji = subcategorySlug
    ? getCatEmoji(subcategorySlug)
    : getCatEmoji(categorySlug)
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      ...
      {/* ── Age Gate — affiché par-dessus tout le contenu ── */}
      {showAgeGate && (
        <AgeGate
          onConfirm={() => {
            setShowAgeGate(false)
          ...
          onRefuse={() => {
            setShowAgeGate(false)
            // Ramener l'utilisateur à la page de recherche sans filtre adulte
            router.push('/search')
          }}
        />
      )}
      ...
      <div className="relative overflow-hidden" style={{ background: "linear-gradient(135deg, #0F1117 0%, #1a2035 60%, #1f1a0e 100%)" }}>
        <div className="relative max-w-7xl mx-auto px-4 py-8 md:px-8">
          <div className="flex items-center gap-2 text-xs text-white/30 mb-4 font-medium">
            ...
            {categorySlug && (
              <Link href={`/search?category=${categorySlug}`} className="hover:text-orange-400 transition-colors text-white/50">
                {getLabel(categorySlug)}
              </Link>
            )}
            {subcategorySlug && (
              <>
                <ChevronRight size={12} />
                <span className="text-white/70">{getLabel(subcategorySlug)}</span>
              </>
            )}
          </div>
            ...
            <div>
              <div className="flex items-center gap-3 mb-1">
                ...
                <h1 className="text-2xl md:text-3xl font-extrabold text-white tracking-tight">
                  {pageLabel}
                  {/* Badge 18+ dans le titre si catégorie adulte confirmée */}
                  {isAdultCategory && ageCleared && (
                    <span className="ml-2 align-middle text-[11px] font-bold bg-pink-600 text-white px-2 py-0.5 rounded-full">
                      🔞 18+
                    </span>
                  )}
                </h1>
            ...
            >
              <div className="flex-1 flex items-center bg-white/10 border border-white/15 rounded-xl overflow-hidden backdrop-blur-sm">
                ...
                <input
                  name="q"
                  defaultValue={q}
                  placeholder={`Rechercher dans ${pageLabel}...`}
                  className="flex-1 px-3 py-2.5 bg-transparent text-white placeholder:text-white/30 text-sm outline-none"
                />
              </div>
              <button type="submit" className="px-4 py-2.5 bg-orange-500 hover:bg-orange-600 text-white text-sm font-semibold rounded-xl transition-colors flex-shrink-0">
                OK
              </button>
            </form>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8">
          <div className="flex items-center gap-2 py-2.5 overflow-x-auto">
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold border transition-all flex-shrink-0 ${hasActiveFilters || showFilters
            ...
            {sortOptions.map((s) => {
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold transition-all flex-shrink-0 ${isActive
            ...
            <div className="ml-auto flex items-center gap-1.5 flex-shrink-0">
              <button
                ...
                className={`p-2 rounded-lg transition-colors ${view === "grid" ? "bg-orange-100 text-orange-500" : "text-gray-400 hover:text-gray-600"}`}
              >
                <GridIcon size={15} />
              </button>
              <button
                onClick={() => setView("list")}
                className={`p-2 rounded-lg transition-colors ${view === "list" ? "bg-orange-100 text-orange-500" : "text-gray-400 hover:text-gray-600"}`}
              >
                <LayoutList size={15} />
              </button>
            </div>
            ...
            <div className="max-w-7xl mx-auto px-4 md:px-8 py-4">
              <div className="flex flex-wrap gap-4 items-end">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {etats.map((e) => (
                      <button
                        key={e}
                        onClick={() => setSelectedEtat(selectedEtat === e ? "" : e)}
                        className={`px-3 py-1.5 text-xs font-medium rounded-xl border transition-all ${selectedEtat === e
                          ? "bg-orange-500 text-white border-orange-500"
                ...
                <div className="flex gap-2 ml-auto">
                  {hasActiveFilters && (
                    <button
                      onClick={() => { setPriceMin(""); setPriceMax(""); setSelectedEtat("") }}
                  ...
                  <button
                    onClick={() => { fetchAds(); setShowFilters(false) }}
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 md:px-8">
        {!loading && !error && ads.length > 0 && ageCleared && (
          <div className="flex items-center justify-between mb-5">
            <p className="text-sm text-gray-500">
              <span className="font-semibold text-gray-800">{ads.length}</span>{" "}
              annonce{ads.length > 1 ? "s" : ""} affichée{ads.length > 1 ? "s" : ""}
              {total > 48 && <span className="text-gray-400"> sur {total.toLocaleString("fr-CI")}</span>}
            </p>
        ...
        {/* Écran de blocage pendant vérification d'âge */}
        {!ageCleared ? (
          <div className="flex flex-col items-center justify-center py-32 text-center">
            <span className="text-6xl mb-4">🔞</span>
            <p className="text-gray-400 text-sm">Vérification de l'âge en cours…</p>
          </div>
        ) : loading ? (
          ...
          <div className="flex flex-col gap-3">
            {ads.map((ad) => <AdCard key={ad.id} ad={ad} view="list" />)}
          </div>
        )}
        ...
        {!loading && ageCleared && total > 48 && (
          <div className="mt-10 text-center">
            <div className="inline-flex items-center gap-3 bg-white border border-gray-200 rounded-2xl px-6 py-4">
              ...
              <p className="text-sm text-gray-500">
                Affichage de <span className="font-bold text-gray-800">48</span> annonces sur{" "}
                <span className="font-bold text-gray-800">{total.toLocaleString("fr-CI")}</span>
              </p>
              <Star size={16} className="text-orange-400" />
            </div>
          </div>

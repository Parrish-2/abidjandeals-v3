...
const SUBCAT_LABELS: Record<string, string> = {
  ...
  'chaussures-bebe': 'Chaussures bébé',
  'jouets-eveil': 'Jouets & Éveil',
  'accessoires-bebe': 'Accessoires bébé',
  'articles-maman': 'Articles pour maman',
  // Épicerie & Produits locaux
  'produits-alimentaires': 'Produits alimentaires',
  'boissons': 'Boissons (eau, jus, sodas...)',
  'produits-locaux': 'Produits locaux (attiéké, huile...)',
  // Sport & Loisirs
                  ...
                  {media.length > 0 && (
                    <div className="grid grid-cols-4 gap-2 mb-3">
                      {media.map((m, i) => {
                        ...
                        return (
                          <div key={i} className={`relative rounded-xl overflow-hidden border-2 aspect-square transition-all ${
                            isUploading ? 'border-orange-500 ring-2 ring-orange-300 ring-offset-1' :
                            i === 0 ? 'border-orange-400' : 'border-gray-100'
                          }`}>
                            {m.type === 'image'
                              ? <img src={m.url} alt="" className={`w-full h-full object-cover ${isUploading ? 'opacity-50' : ''}`} />
                              : <video src={m.url} className={`w-full h-full object-cover ${isUploading ? 'opacity-50' : ''}`} muted />}
                            {isUploading && (
                    ...
                    <div className="mb-3 bg-orange-50 border border-orange-100 rounded-xl px-4 py-3">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-semibold text-orange-700">{uploadProgress.label}</span>
                        </div>
                        <span className="text-xs font-bold text-orange-500">{uploadProgress.current}/{uploadProgress.total}</span>
                      </div>
                      ...
                      <p className="text-[10px] text-orange-400 mt-1.5">Envoi en cours — ne fermez pas la page</p>
                    </div>
                  )}
